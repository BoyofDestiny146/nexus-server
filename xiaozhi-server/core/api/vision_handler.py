...
class VisionHandler:
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            # careconnect: persist the captured frame so the dashboard can show it
            # in the conversation. Saved to the shared cc-photos volume (served by
            # Caddy at /photos/); the connection hook logs a chat turn referencing
            # this URL. Best-effort — never fail the vision response on a save error.
            photo_url = None
            try:
                import os as _os
                import uuid as _uuid

                photos_dir = _os.environ.get("CC_PHOTOS_DIR", "/photos")
                _os.makedirs(photos_dir, exist_ok=True)
                pid = _uuid.uuid4().hex
                with open(_os.path.join(photos_dir, f"{pid}.jpg"), "wb") as _f:
                    _f.write(image_data)
                photo_url = f"/photos/{pid}.jpg"
            except Exception as _pe:
                self.logger.bind(tag=TAG).warning(
                    f"careconnect: failed to save vision photo (non-fatal): {_pe}"
                )
