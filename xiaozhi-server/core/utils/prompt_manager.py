...
class PromptManager:
    ...
    # careconnect: per-device verbosity directive appended to the system prompt.
    _RESPONSE_LENGTH_DIRECTIVE = {
        "brief": (
            "\n\n# RESPONSE LENGTH — HARD RULE (overrides everything above)\n"
            "Your ENTIRE reply MUST be ONE or TWO short sentences. Never more. "
            "Maximum ~35 words total. No lists. No preamble. No 'As for…' digressions. "
            "Do not cover multiple topics — answer the single most important thing, then STOP. "
            "If the user asked several things, answer briefly and invite them to pick one."
        ),
        "normal": (
            "\n\n# RESPONSE LENGTH — HARD RULE (overrides everything above)\n"
            "Keep your reply SHORT: at most THREE short sentences (~60 words). "
            "Be warm and conversational, but get to the point and then STOP. "
            "No lists, no preamble, no digressions."
        ),
        "detailed": (
            "\n\n# RESPONSE LENGTH\nYou may give a fuller answer, but stay focused "
            "and break long content into short segments."
        ),
    }
    ...
    def build_enhanced_prompt(
        ...
        if not self.base_prompt_template:
            base = user_prompt
            return base + self._RESPONSE_LENGTH_DIRECTIVE.get(response_length, "")
        ...
        try:
            ...
            # careconnect: append the per-device verbosity directive
            enhanced_prompt += self._RESPONSE_LENGTH_DIRECTIVE.get(response_length, "")
            device_cache_key = f"device_prompt:{device_id}"
            self.cache_manager.set(
