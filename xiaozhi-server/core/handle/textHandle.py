            ...
            if msg_json["state"] == "start":
                ...
                if "text" in msg_json:
                    ...
                    if is_wakeup_words and not enable_greeting:
                        ...
                        # careconnect: English wake greeting. The detected wake
                        # word (e.g. "Jarvis") must NOT be forwarded to the LLM as
                        # a user query; instead nudge a brief greeting in the
                        # client's own persona (the assistant is "Haizel").
                        _cc_wake = "Please greet me warmly in one short sentence."
                        enqueue_asr_report(conn, _cc_wake, [])
                        await startToChat(conn, _cc_wake)
                    else:
