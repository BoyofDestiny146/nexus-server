...
def _careconnect_find_set_volume_tool(mcp_client: "MCPClient"):
    """careconnect: locate the device's set-volume tool by name, robustly.

    Upstream hard-codes ``self.audio_speaker.set_volume`` but the SenseCAP /
    SenseCAP-Watcher fork may expose it under a different prefix (e.g.
    ``self.speaker.set_volume`` or just ``set_volume``). Match any tool whose
    ORIGINAL (un-sanitized) name ends with or contains ``set_volume`` and
    return that original name (call_mcp_tool expects the original name).
    Returns None if no such tool exists.
    """
    # name_mapping: sanitized_name -> original_name
    originals = list(mcp_client.name_mapping.values()) or list(mcp_client.tools.keys())
    # Prefer an exact suffix match, then a substring match.
    for orig in originals:
        if orig.endswith("set_volume") or orig.endswith(".set_volume"):
            return orig
    for orig in originals:
        if "set_volume" in orig:
            return orig
    return None


async def _careconnect_set_default_volume(conn, mcp_client: "MCPClient", volume: int = 95):
    """careconnect: nudge the device speaker to a known-audible volume right
    after MCP becomes ready. The device's factory default is sometimes near
    silent; without this the user perceives a "broken speaker"."""
    tool_name = _careconnect_find_set_volume_tool(mcp_client)
    if not tool_name:
        logger.bind(tag=TAG).info(
            "careconnect: no set_volume tool on this device; skipping volume nudge"
        )
        return
    try:
        ...
        logger.bind(tag=TAG).info(
            f"careconnect set_volume({volume}) via {tool_name} result: {str(result)[:120]}"
        )
    except Exception as e:
        logger.bind(tag=TAG).warning(
            f"careconnect set_volume({volume}) via {tool_name} failed (non-fatal): {e}"
        )
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                for i, tool in enumerate(tools_data):
                    ...
                    # careconnect: log device tool names at INFO so the real
                    # set_volume tool name is always visible in deploy logs
                    # (needed to confirm the robust set_volume matcher).
                    logger.bind(tag=TAG).info(f"客户端工具 #{i+1}: {name}")
                ...
                if next_cursor:
                    ...
                    # careconnect: ensure the device speaker is at an audible
                    # default. Without this the device default may be very low
                    # which is heard as "messed up" / barely audible audio.
                    # Best-effort, fire-and-forget — failures are logged not
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).warning(
                            f"careconnect set_volume schedule failed: {e}"
                        )
            return
...
async def call_mcp_tool(
    ...
    # careconnect: resolve the tool name robustly. The device advertises tools
    # under their ORIGINAL names (e.g. "self.audio_speaker.set_volume") but the
    # client stores them keyed by the SANITIZED name (dots -> underscores, via
    # sanitize_tool_name) in mcp_client.tools, with name_mapping mapping
    # sanitized -> original. Callers (the LLM, and our set_volume nudge) may
    # pass EITHER form, so accept both:
    #   - already-sanitized key present in tools  -> use as-is
    #   - original name -> sanitize it to find the stored key
    # Then look up the original name from name_mapping for the on-wire payload.
    # Without this, passing the original "self.audio_speaker.set_volume" failed
    # has_tool() (which checks the sanitized-keyed dict) -> "工具 ... 不存在".
    if mcp_client.has_tool(tool_name):
        lookup_key = tool_name
    else:
        sanitized = sanitize_tool_name(tool_name)
        if mcp_client.has_tool(sanitized):
            lookup_key = sanitized
        else:
    ...
    actual_name = mcp_client.name_mapping.get(lookup_key, lookup_key)
    payload = {
        "jsonrpc": "2.0",
        "id": tool_call_id,
        "method": "tools/call",
