...
# careconnect: the deployed Piper voice is English-only. If the LLM ever emits
# CJK / Japanese / Korean glyphs or emoji, the English voice garbles them into
# noise (this was the "broken speaker" symptom). Strip any non-English script
# (and emoji) before the text ever reaches synthesis. This is the last line of
# defense behind the model choice (llama3.1) and the English-only persona pins.
_NON_ENGLISH_TTS_RE = re.compile(
    "["
    ...
    "ᄀ-ᇿ"   # Hangul Jamo
    "＀-￯"   # Fullwidth/Halfwidth forms (incl. CJK punctuation)
    "　-〿"   # CJK Symbols and Punctuation
    "\U0001f000-\U0001ffff"  # emoji / pictographs / symbols
    "\U0001f900-\U0001faff"
    "☀-➿"   # misc symbols + dingbats (common emoji)
    "]+",
    flags=re.UNICODE,
)


def strip_non_english_for_tts(text):
    """Remove CJK/JP/KR glyphs and emoji so the English voice never sees them.

    Returns the cleaned string. Caller decides whether the remainder is too
    short to be worth synthesizing.
    """
    if not text:
        return text
    cleaned = _NON_ENGLISH_TTS_RE.sub("", text)
    # collapse whitespace left behind by stripping
    cleaned = re.sub(r"[ \t]{2,}", " ", cleaned).strip()
    if cleaned != text:
        logger.bind(tag=TAG).warning(
            f"TTS非英文字符已剥离: {text!r} -> {cleaned!r}"
        )
    return cleaned
...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        self.punctuations = (
            ...
            "：",
            "~",
            ".",  # careconnect: English period — without it, long English
                  # paragraphs (all periods) never segment until end-of-stream,
                  # so the whole reply becomes one oversized TTS chunk that
                  # exceeds tts_timeout and the audio stalls (text still shows).
        )
        self.first_sentence_punctuations = (
            ...
            "；",
            ";",
            ".",  # careconnect: English period (see note above)
            "：",
        )
    ...
    def to_tts(self, text):
        ...
        # careconnect: drop any non-English script / emoji before synthesis.
        text = strip_non_english_for_tts(text)
        # If stripping leaves nothing meaningful (no ASCII letters/digits),
        # skip this segment entirely rather than feeding the voice junk.
        if not text or not re.search(r"[A-Za-z0-9]", text):
            logger.bind(tag=TAG).info("TTS段落剥离后为空，跳过合成")
            return None
        max_repeat_time = 5
